...
function Context:type_check_index(anode, bnode, a, b)
   ...
   local tuple_types = type(ra) == "table" and ra.types or nil
   if tuple_types and rb.typename == "integer" then
      if bnode.constnum then
         if bnode.constnum >= 1 and bnode.constnum <= #tuple_types and bnode.constnum == math.floor(bnode.constnum) then
            return tuple_types[bnode.constnum]
         ...
         local type_list = tuple_types
         if ra.fields and ra.elements then
            type_list = { ra.elements }
            for _, child in ipairs(tuple_types) do
                  ...
                  if can_split_on_comma and
                     ...
                     if st and st.kind == "local_declaration" then
                        ...
                        if vlist and decl then
                           local typelist = decl[1]
                           if typelist and typelist.kind == "..." then
                              typelist = decl[2]
                           end
