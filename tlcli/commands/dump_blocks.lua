local _tl_compat; if (tonumber((_VERSION or ''):match('[%d.]*$')) or 0) < 5.3 then local p, m = pcall(require, 'compat53.module'); if p then _tl_compat = m end end; local io = _tl_compat and _tl_compat.io or io; local ipairs = _tl_compat and _tl_compat.ipairs or ipairs; local math = _tl_compat and _tl_compat.math or math; local os = _tl_compat and _tl_compat.os or os; local pairs = _tl_compat and _tl_compat.pairs or pairs; local string = _tl_compat and _tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local type = type
...
local function write_json_block(fd, bl, indent)
   ...
   for _, idx in ipairs(child_indexes(bl)) do
      ...
      table.insert(entries, { key = string.format("%q", tostring(idx)), val = val })

      local name = edge_name(bl.kind, idx)
      if name then
         table.insert(entries, { key = string.format("%q", name), val = val })
      end
   end
